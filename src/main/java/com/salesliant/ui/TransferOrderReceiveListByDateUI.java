...
import com.salesliant.report.TransferOrderReceiveListByDateReportLayout;
...
public class TransferOrderReceiveListByDateUI extends BaseListUI<TransferOrderHistory> {
    ...
    private static final Logger LOGGER = Logger.getLogger(TransferOrderReceiveListByDateUI.class.getName());

    public TransferOrderReceiveListByDateUI() {
        loadData();
    ...
    private void loadData() {
        ...
        Platform.runLater(() -> {
            Map<String, List<TransferOrderHistory>> groupByDate = daoTransferOrderHistory.readOrderBy(TransferOrderHistory_.receiveStore, Config.getStore(), TransferOrderHistory_.dateReceived, AppConstants.ORDER_BY_ASC)
                    .stream()
                    .filter(e -> e.getDateReceived() != null)
                    .collect(Collectors.groupingBy(e -> getDateDateFormat().format(e.getDateReceived())));
            fList = new ArrayList<>(groupByDate.values())
                    .stream()
                    .sorted((e1, e2) -> e2.get(0).getDateReceived().compareTo(e1.get(0).getDateReceived())).collect(Collectors.toList());
        ...
        switch (code) {
            case AppConstants.ACTION_SELECT:
                ...
                selectBtn.addEventFilter(ActionEvent.ACTION, event -> {
                    ...
                    List<List<TransferOrderHistory>> listList = fList
                            .stream()
                            .filter(e -> e.get(0).getDateReceived().after(from) && e.get(0).getDateReceived().before(to))
            ...
            case AppConstants.ACTION_PRINT:
                if (!fSummaryList.isEmpty()) {
                    TransferOrderReceiveListByDateReportLayout layout = new TransferOrderReceiveListByDateReportLayout(fSummaryList);
                    try {
    ...
    private Node createMainView() {
        ...
        monthCol.setCellValueFactory((TableColumn.CellDataFeatures<List<TransferOrderHistory>, String> p) -> {
            if (p.getValue() != null && !p.getValue().isEmpty()) {
                String year = getDateDateFormat().format(p.getValue().get(0).getDateReceived());
        ...
        monthToDateAmountCol.setCellValueFactory((TableColumn.CellDataFeatures<List<TransferOrderHistory>, String> p) -> {
            if (p.getValue() != null && !p.getValue().isEmpty()) {
                LocalDate date = p.getValue().get(0).getDateReceived().toLocalDateTime().toLocalDate();
