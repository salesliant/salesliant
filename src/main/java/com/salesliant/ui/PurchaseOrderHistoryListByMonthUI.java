...
import com.salesliant.report.PurchaseOrderHistoryListByMonthReportLayout;
...
public class PurchaseOrderHistoryListByMonthUI extends BaseListUI<PurchaseOrderHistory> {
    ...
    private static final Logger LOGGER = Logger.getLogger(PurchaseOrderHistoryListByMonthUI.class.getName());

    public PurchaseOrderHistoryListByMonthUI() {
        loadData();
    ...
    private void loadData() {
        ...
        Platform.runLater(() -> {
            Map<String, List<PurchaseOrderHistory>> groupByMonth = daoPurchaseOrderHistory.readOrderBy(PurchaseOrderHistory_.store, Config.getStore(), PurchaseOrderHistory_.datePurchased, AppConstants.ORDER_BY_ASC)
                    .stream()
                    .filter(e -> e.getDatePurchased() != null)
                    .collect(Collectors.groupingBy(e -> getMonthDateFormat().format(e.getDatePurchased())));
            fList = new ArrayList<>(groupByMonth.values())
                    .stream()
    ...
    public void handleAction(String code) {
        switch (code) {
            ...
            case AppConstants.ACTION_PRINT:
                if (!fList.isEmpty()) {
                    PurchaseOrderHistoryListByMonthReportLayout layout = new PurchaseOrderHistoryListByMonthReportLayout(fList);
                    try {
    ...
    private Node createMainView() {
        ...
        TableColumn<List<PurchaseOrderHistory>, String> monthCol = new TableColumn<>("Month");
        ...
        TableColumn<List<PurchaseOrderHistory>, String> amountCol = new TableColumn<>("Year To Date");
