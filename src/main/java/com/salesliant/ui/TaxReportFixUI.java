...
public class TaxReportFixUI extends BaseListUI<Invoice> {
                        ...
                        if (invoice.getInvoiceEntries() != null && !invoice.getInvoiceEntries().isEmpty()) {
                            for (InvoiceEntry ie : invoice.getInvoiceEntries()) {
                                if (ie.getItemLookUpCode() != null
                                    ...
                                    TaxClass tc = getTaxClass(ie);
                                    if (tc != null) {
                                        BigDecimal rate = getTax(tc, tz).getTaxRate().getRate();
                                        if (taxRate.compareTo(BigDecimal.ZERO) == 0) {
                                            rate = BigDecimal.ZERO;
                                        }
                                        BigDecimal entryAmount = zeroIfNull(ie.getPrice()).multiply(zeroIfNull(ie.getQuantity()));
                                        BigDecimal taxAmount = entryAmount.multiply(rate);
                                        ie.setTaxAmount(taxAmount);
    ...
    private HBox createClonePrintExportCloseButtonPane() {
        Button fixButton = ButtonFactory.getButton(ButtonFactory.BUTTON_SELECT, AppConstants.ACTION_SELECT, "Fix Tax", fHandler);
