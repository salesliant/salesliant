...
import com.salesliant.report.InvoiceListByYearReportLayout;
...
public class InvoiceListByYearUI extends BaseListUI<Invoice> {
    ...
    private static final Logger LOGGER = Logger.getLogger(InvoiceListByYearUI.class.getName());

    public InvoiceListByYearUI() {
        loadData();
    ...
    private void loadData() {
        ...
        Platform.runLater(() -> {
            Map<String, List<Invoice>> groupByYear = daoInvoice.readOrderBy(Invoice_.store, Config.getStore(), Invoice_.dateInvoiced, AppConstants.ORDER_BY_ASC)
                    .stream()
                    .filter(e -> e.getDateInvoiced() != null)
                    .collect(Collectors.groupingBy(e -> getYearDateFormat().format(e.getDateInvoiced())));
            List<List<Invoice>> list = new ArrayList<>(groupByYear.values())
                    .stream()
        ...
        switch (code) {
            case AppConstants.ACTION_PRINT:
                if (!fSummaryList.isEmpty()) {
                    InvoiceListByYearReportLayout layout = new InvoiceListByYearReportLayout(fSummaryList);
                    try {
    ...
    private Node createMainView() {
        ...
        TableColumn<List<Invoice>, String> yearCol = new TableColumn<>("Year");
        yearCol.setCellValueFactory((TableColumn.CellDataFeatures<List<Invoice>, String> p) -> {
            if (p.getValue() != null && !p.getValue().isEmpty()) {
                String year = getYearDateFormat().format(p.getValue().get(0).getDateInvoiced());
                return new SimpleStringProperty(year);
            } else {
                return null;
            }
        });
        yearCol.setCellFactory(stringCell(Pos.CENTER_LEFT));
        yearCol.setPrefWidth(140);
        ...
        numberOfReturnCol.setPrefWidth(150);
        ...
        fSummaryTable.getColumns().add(yearCol);
