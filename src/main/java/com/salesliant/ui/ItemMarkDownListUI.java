...
import com.salesliant.report.ItemMarkDownListReportLayout;
...
public class ItemMarkDownListUI extends BaseListUI<ItemLog> {
    ...
    private static final Logger LOGGER = Logger.getLogger(ItemMarkDownListUI.class.getName());

    public ItemMarkDownListUI() {
        setDefaultDate();
        loadData();
    ...
    private void loadData() {
        ...
        Platform.runLater(() -> {
            List<ItemLog> list = daoItemLog.readBetweenDate(ItemLog_.store, Config.getStore(), ItemLog_.transferType, DBConstants.TYPE_ITEMLOG_TRANSFERTYPE_INVOICE, ItemLog_.dateCreated, fFrom, fTo)
                    .stream()
                    .filter(e -> e.getInvoice() != null && e.getItemPrice() != null && e.getPrice() != null && e.getPrice().compareTo(e.getItemPrice()) < 0)
        ...
        switch (code) {
            ...
            case AppConstants.ACTION_PRINT:
                if (!fEntityList.isEmpty()) {
                    ItemMarkDownListReportLayout layout = new ItemMarkDownListReportLayout(fEntityList, fFrom, fTo);
                    try {
    ...
    private Node createMainView() {
        ...
        TableColumn<ItemLog, String> itemPriceCol = new TableColumn<>("Item Price");
        itemPriceCol.setCellValueFactory(new PropertyValueFactory<>(ItemLog_.itemPrice.getName()));
        itemPriceCol.setCellFactory(stringCell(Pos.CENTER_RIGHT));
        itemPriceCol.setPrefWidth(90);

        TableColumn<ItemLog, String> priceCol = new TableColumn<>("Sales Price");
        priceCol.setCellValueFactory(new PropertyValueFactory<>(ItemLog_.price.getName()));
