...
public class ItemBackOrderListReportLayout {

    private final List<Item> fList;

    public ItemBackOrderListReportLayout(List<Item> list) {
        this.fList = list;
    }
    ...
    public JasperReportBuilder build() throws DRException {
        ...
        TextColumnBuilder<BigDecimal> qtyColumn = col.column("Quantity", "qty", decimalType).setTitleStyle(rightColumnTitleStyle).setStyle(rightColumnStyle).setFixedWidth(130);

        report.setTemplate(Templates.invoiceReportTemplate)
                .columns(skuColumn, descriptionColumn, qtyColumn)
                .title(createTitleComponent("Item Back Order List"),
    ...
    public JRDataSource createDataSource() {
        DRDataSource dataSource = new DRDataSource("sku", "description", "qty");
        if (!fList.isEmpty()) {
