...
import static com.salesliant.util.BaseListUI.getQuantityOnRMA;
...
public class ItemOnRMAListReportLayout {

    private final List<Item> fList;

    public ItemOnRMAListReportLayout(List<Item> list) {
        this.fList = list;
    }
    ...
    public JasperReportBuilder build() throws DRException {
        ...
        TextColumnBuilder<BigDecimal> qtyOnRMAColumn = col.column("Qty On RMA", "qtyOnRMA", decimalType).setTitleStyle(rightColumnTitleStyle).setStyle(rightColumnStyle).setFixedWidth(130);

        report.setTemplate(Templates.invoiceReportTemplate)
                .columns(skuColumn, descriptionColumn, qtyOnRMAColumn)
                .title(createTitleComponent("Item On RMA List"),
    ...
    public JRDataSource createDataSource() {
        DRDataSource dataSource = new DRDataSource("sku", "description", "qtyOnRMA");
        if (!fList.isEmpty()) {
            fList.stream().forEach((p) -> {
                dataSource.add(p.getItemLookUpCode(), p.getDescription(), getQuantityOnRMA(p));
            });
        }
        return dataSource;
    }
