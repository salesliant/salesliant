...
public class TransferOrderReceiveListByDateReportLayout {

    private final List<List<TransferOrderHistory>> fList;

    public TransferOrderReceiveListByDateReportLayout(List<List<TransferOrderHistory>> list) {
        this.fList = list;
    }
    ...
    public JasperReportBuilder build() throws DRException {
        ...
        report.setTemplate(Templates.invoiceReportTemplate)
                ...
                .title(createTitleComponent("Transfer Receive By Date"),
        ...
        if (!fList.isEmpty()) {
            fList.stream().forEach((p) -> {
                ...
                BigDecimal monthAmount = fList
                        ...
                        .reduce(BigDecimal.ZERO, BigDecimal::add);
                dataSource.add(getDateDateFormat().format(p.get(0).getDateReceived()), qty, total, monthAmount);
            });
        }
        return dataSource;
    }
